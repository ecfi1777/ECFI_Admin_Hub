...
import type { UpcomingWorkItem, UpcomingWorkWeekNote } from "./types";
...
export function useUpcomingWork(anchorDate: Date) {
  ...
  const itemsForWeek = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []) as unknown as UpcomingWorkItem[];
    },
  ...
  const weekNote = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .maybeSingle();
      if (error) throw error;
      return data as UpcomingWorkWeekNote | null;
    },
    enabled: !!organizationId,
  });
